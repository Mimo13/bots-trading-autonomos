...
def recommend_for_bot(conn, bot:str):
    ...
    confidence=0.55
    if n < 20:
        recs.append({'type':'data','action':'Aumentar muestra mínima a 50 trades antes de tocar parámetros','risk':'low'})
        confidence=0.45
    if pnl7 < 0:
        recs.append({'type':'risk','action':'Reducir max_risk_per_trade un 10-20% para mañana','risk':'medium'})
        recs.append({'type':'filter','action':'Subir edge_min +0.005 temporalmente','risk':'medium'})
    if win_rate < 0.45 and n >= 20:
        recs.append({'type':'quality','action':'Endurecer tv_min_confidence +0.03','risk':'medium'})
    if pnl7 > 0 and win_rate > 0.55 and n >= 20:
        recs.append({'type':'scale','action':'Mantener parámetros; evaluar incremento de stake +5% solo en paper','risk':'low'})
        confidence=0.7

    if not recs:
        recs.append({'type':'hold','action':'Mantener configuración actual y seguir monitorización','risk':'low'})
